...
export default new Runtime({
  apply({bundle, bundleGraph}) {
    ...
    for (let dependency of bundleGraph.getDependenciesInBundle(bundle)) {
    ...
    for (let dependency of asyncDependencies) {
      let resolved = bundleGraph.resolveExternalDependency(bundle, dependency);
      if (resolved == null) {
        continue;
      }
      ...
      if (resolved.type === 'asset') {
        ...
        // return a simple runtime of `Promise.resolve(parcelRequire("assetId"))`.
        assets.push({
          filePath: __filename,
